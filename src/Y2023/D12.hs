...
countSlots [] _ = 0
countSlots (size : dgs_rest) scs
  | remaining_space_needed > remaining_space = 0
  ...
  where
    first_damaged = V.findIndex (== Damaged) scs
...
countRecord sr = trace (show "Processing...") numSlots
  where
    split_record = (sr.damageGroups, sr.conditions V.++ V.fromList [Operational])
...
unfoldRecord r = SpringRecord unfolded_conditions unfolded_damage_groups
  where
    unfolded_conditions = V.concat $ intersperse (V.fromList [Unknown]) (replicate 5 r.conditions)
...
partOne srs = trace (show record_counts) $ sum record_counts
  where
    record_counts = map countRecord srs
