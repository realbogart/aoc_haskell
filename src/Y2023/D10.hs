...
countContained row = result
  where f :: (Bool, Char, Int) -> Char -> (Bool, Char, Int) 
        f (isContained, l, c) currentTile | currentTile == '|' = (not isContained, ' ', c)
...
getGridValueReplaceStart g (cx, cy) = 
  case tile of
    'S' -> getStartTile
    _ -> tile
  where tile = getGridValue g (cx, cy)
        rightTile   = getGridValue g (cx + 1, cy)
        belowTile   = getGridValue g (cx, cy + 1)
        leftTile    = getGridValue g (cx - 1, cy)
        aboveTile   = getGridValue g (cx, cy - 1)
        ...
        getStartTile  | connectRight && connectBelow = 'F' 
