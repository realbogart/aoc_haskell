...
getRangeIntersection from to  | from.source + from.count <= to.source = Nothing
                              | from.source >= to.source + to.count = Nothing
                              | from.source <= to.source = Just $ Range 0 
                                                                        to.destination
                                                                        (right - to.source)
                              | to.source <= from.source = Just $ Range 0
                                                                        (to.destination + (from.source - to.source)) 
                                                                        (right - from.source)
                              | otherwise = error "Should not be here"
                              where right = min (from.source + from.count) (to.source + to.count)
...
getRangeGaps target ranges = getRangeIntersections target ( getFirst sorted_ranges ++ 
                                                            getLast sorted_ranges ++
                                                            mapMaybe getGap range_pairs)
  where sorted_ranges = sortRangesBySource ranges
        range_pairs = zip sorted_ranges (tail sorted_ranges)
        getFirst :: [Range] -> [Range]
        getFirst (f:_)  | target.source < f.source = [Range target.source target.source (f.source - target.source)]
                        | otherwise = []
        getFirst _ = []
        getLast :: [Range] -> [Range]
        getLast [] = []
        getLast rs  | (target.source + target.count) > l_right = [Range l_right 
                                                                        l_right 
        ...
        getGap (a,b)  | a.source + a.count < b.source = Just $ Range  (a.source + a.count) 
                                                                      (a.source + a.count)
...
mapRange targets m = concatMap transformValueRange targets
  where transformValueRange t = getRangeIntersections t m.ranges ++ 
...
getSeedRanges = map getSeedRange . chunksOf 2
  where getSeedRange [start, c] = Range 0 start c
