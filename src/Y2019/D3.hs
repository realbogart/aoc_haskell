...
getAllIntersectionsForLine l = catMaybes . map (getIntersection l)
...
getAllIntersections :: [Line] -> [Line] -> [Vec2]
getAllIntersections wiresA wiresB = concatMap (`getAllIntersectionsForLine` wiresB) wiresA

vec2Dist (Vec2 x y) = abs x + abs y

partOne = minimum . map vec2Dist . filter (/= Vec2 0 0) . uncurry getAllIntersections . (getWires . map getLines)
  where getWires [wiresA, wiresB] = (wiresA, wiresB)
        getWires _ = error "Invalid wires"
