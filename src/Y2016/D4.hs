...
isReal r = commonStr (room_encrypted r) == room_checksum r
...
partOne = sum . map room_id . filter isReal
partTwo = map applyCaesar . filter isReal
  where applyCaesar r = show (room_id r) ++ ": " ++ map (caesar (room_id r)) (room_encrypted r) 
