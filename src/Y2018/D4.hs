...
cleanEntryData = scanl1 copyGuardID . sort
  where
    copyGuardID _ current@EntryData {entry_type = ShiftStart} = current
...
getSleepShifts = map getSleepShift . chunksOf 2 . getSleepEntries
  where
    getSleepShift [sleep, awake] = SleepShift sleep.guard_id sleep.timestamp.minute awake.timestamp.minute
...
partTwo l =
  answer $
    maximumBy (\(_, _, a) (_, _, b) -> compare a b) $
      map
        ((`getMostSleptMinuteForID` sleepShifts) . (\SleepShift {sleep_guard_id = gid} -> gid) . head)
        (groupGuards sleepShifts)
  where
    sleepShifts = getSleepShifts l
    answer (x, y, _) = x * y
